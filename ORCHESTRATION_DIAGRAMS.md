...
│   │       **CRITICAL: execute() MUST be idempotent**
│   │
│   ├── orchestrator.py             [MAIN CONDUCTOR]
│   │   └── Class: OrchestratorAgent(BaseAgent)
│   │       Manages: Workflow state, agent registry, handoffs
│   │       States: IDLE, WAITING_FOR_NEXT_CYCLE, FETCHING_DATA,
│   │               ANALYZING_MARKET, BACKTESTING, RISK_ASSESSMENT,
│   │               EXECUTING, MONITORING, ERROR, PAUSED
│   │       Tracks: workflow_history (stage transitions),
│   │               workflow_trace (complete agent messages)
│   │
...
│   │       Method: execute(all_results, workflow_trace) → logs + alerts
│   │       Output: logs/trading_bot.log, logs/events.jsonl
│   │       Receives: Complete workflow_trace for full auditability
│   │
│   └── auditor.py                  [Agent 7 - Safety Verification]
│       └── Class: AuditorAgent(BaseAgent)
│           Method: execute(workflow_trace) → audit_passed + violations
│           Purpose: Last line of defense - re-validates all safety checks
│           Triggers: Circuit breaker on violations
│
...
```
         ...
         (IDLE, WORKING,       │
          ERROR, PAUSED)       │
              ...
              ┌────────────┼────────────┬──────────────┐
              │            │            │              │
        BacktestingAgent ExecutionAgent MonitoringAgent AuditorAgent
        (Validation)      (Trading)     (Logging)      (Safety Audit)
```
...
```
    ...
    └────────────┬─────────────────────────┘
                 ↓
    POST-CYCLE SAFETY AUDIT
        ↓
    ┌──────────────────────────────────────┐
    │  [7] AuditorAgent.execute()          │
    │      Input: {workflow_trace}         │
    │      Output: {audit_passed: True,    │
    │               violations: [...]}     │
    │      Safety: Re-validates all checks │
    └────────────┬─────────────────────────┘
                 │
        ┌────────┴──────────────┐
        │ audit_passed = TRUE?  │
        │                       │ NO → activate_circuit_breaker()
        │                       └─────→ ERROR state → END
        │ YES
        ↓
    WORKFLOW STAGE: WAITING_FOR_NEXT_CYCLE
        ↓
     SUCCESS → END / REPEAT
```
...
When orchestrator needs to execute an agent:
    orchestrator._execute_agent_phase(
        ...
        action='assess_and_size_position',**[IDEMPOTENT]** |
| **OrchestratorAgent** | Agent | Main conductor | `execute(symbols)` |
| **WorkflowStage** | Enum | State machine states | 10 states (added WAITING_FOR_NEXT_CYCLE) |
| **AgentStatus** | Enum | Agent status | 4 states |
| **agent_registry** | Dict | Store agents | `register_agent()` |
| **workflow_trace** | List | Complete agent messages | Full audit trail |
| **workflow_history** | List | Stage transitions | State machine log |
| **_execute_agent_phase()** | Method | Call agents | Handoff coordination |
| **Message Format** | Dict | Standardized output | `create_message()` |
| **AuditorAgent** | Agent | Safety verification | `execute(workflow_trace)` |

✅ **Complete orchestration topology confirmed**  
✅ **Production enhancements implemented (Feb 10, 2026)
```
