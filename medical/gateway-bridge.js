 ...
 *
 * Registers 5 medical agents with the gateway and routes incoming
 * messages through the pipeline, returning processed results.
 *
 * Usage:
...
function registerAgents() {
  for (const def of AGENT_DEFINITIONS) {
    const capabilities = getAgentCapabilities(def.role);
    const registration = {
      type: 'agent_register',
      agent: {
        ...
        name: def.name,
        role: def.role,
        capabilities: capabilities ? capabilities.tasks : [],
...
function handleAgentQuery(msg) {
  ...
  send({
    ...
    role: agentDef.role,
    capabilities: getAgentCapabilities(agentDef.role),
    status: 'active'
  });
}
